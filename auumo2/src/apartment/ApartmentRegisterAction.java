...
public class ApartmentRegisterAction extends Action {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		apbean.setFloor_plan_image(bytes);						//7 間取りの画像
		apbean.setPet_friendly(pet_friendly);					//10 ペット可否
		apbean.setApartment_image(bytes2);						//11 物件画像
...
//空文字のStringをintに変換するとエラーが出るため、String型で入力チェックして変換
		...
		if ( error_message.length() > 0) {
			//チェックエラーあり
			session.setAttribute("apbean", apbean);
			session.setAttribute("ap_reg_error_message", error_message);
			return "ap_reg.jsp";
		} else {
			//チェックエラーなしならbeanに家賃、敷金礼金をセット
			session.setAttribute("apbean", apbean);
			return "apregistercheck.jsp";
		}
	}
