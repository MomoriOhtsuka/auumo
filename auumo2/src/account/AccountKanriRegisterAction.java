...
public class AccountKanriRegisterAction extends Action {
   ...
   public String execute(HttpServletRequest request, HttpServletResponse response)
		...
		String login_id = request.getParameter("login_id");		//ログインID
		String nick_name = request.getParameter("nick_name");	//ニックネーム
		String password = request.getParameter("password");		//パスワード
		String email_address = request.getParameter("email_address");	//メールアドレス
    	...
    	if(error_message.length()==0) {
    		//チェックエラーなし
    		session.setAttribute("kanri_reg_usermstbean", us);
    		return "ac_kanri_insert_confirm.jsp";
    	} else {
    		//チェックエラーあり
    		error_message.append("</br>");
    		session.setAttribute("kanri_reg_usermstbean", us);
    		session.setAttribute("kanri_reg_error_message",error_message );
    		return "ac_kanri_reg.jsp";
    	}
    }
