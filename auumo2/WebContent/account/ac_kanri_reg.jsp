...
<div class= wrapper>
	<form action="AccountKanriRegister.action" method="post" name="UserMst">
		...
		<!-- 上のほう -->
	<h3>
	<c:if test="${!empty kanri_reg_error_message}">
		${kanri_reg_error_message}
	</c:if>
	</h3>
		<div class=inputs>
			<label class=label>ニックネーム<span class=necessary>必須</span>
			<span class= note>※1文字以上10文字以内</span></label><br>
			<input class=input_form type="text" name="nick_name" min="1" maxlength="10"  value="${kanri_reg_usermstbean.nick_name}"
		...
		<div class=inputs>
			...
			<input class=input_form type="text" name="login_id" pattern="^[a-zA-Z0-9]+$" min="5" maxlength="10"  value="${kanri_reg_usermstbean.login_id}" placeholder="例）suumo0831" required><span class="error">※入力がありません</span><br>
		</div>
		...
		<div class=inputs>
			...
			<span class= note>※半角英数字5文字以上10文字以内</span></label><br>
				...
				class=input_form value="${kanri_reg_usermstbean.password}" placeholder="例）123456AabC" required>
		...
		<div class=inputs>
			...
			<input class=input_form type="email" name="email_address" value="${kanri_reg_usermstbean.email_address}"
